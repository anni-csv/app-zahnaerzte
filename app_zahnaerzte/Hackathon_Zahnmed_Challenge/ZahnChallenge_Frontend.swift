...
// Note: Color extension and AppState are defined in `ZahnChallenge_Backend.swift` to avoid duplicates.
...
struct Step1FreeTextView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ProgressBarView(step: 1, total: 7, score: 0)
        ...
        .navigationTitle("Schritt 1 von 7")
        .navigationBarTitleDisplayMode(.inline)
        .background(Color(hex: "F9FAFB"))
    }
}
...
struct Step2MCQView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ProgressBarView(step: 2, total: 7, score: submitted ? 10 : 0)
                ...
                // Statt: Text(submitted ? "Weiter →" : "Antwort bestätigen")
                // So:
                if submitted {
        ...
        .navigationTitle("Schritt 2 von 7")
        .navigationBarTitleDisplayMode(.inline)
        .background(Color(hex: "F9FAFB"))
    }
...
// MARK: - STEP 3: SORTING (Aufbewahrung)
...
struct Step3SortingView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ProgressBarView(step: 3, total: 7, score: score)
        ...
        .navigationTitle("Schritt 3 von 7")
        .navigationBarTitleDisplayMode(.inline)
        .background(Color(hex: "F9FAFB"))
    ...
    private func isCorrect(_ item: SortItem) -> Bool {
        rankOf(item) == item.correctRank
    }
...
struct SortRowView: View {
    ...
    var borderColor: Color {
        if !submitted { return Color(hex: "E5E7EB") }
        return isCorrect ? Color.green : Color.red
    }
    var bgColor: Color {
        if !submitted { return Color.white }
        return isCorrect ? Color(hex: "CAFFBF") : Color(hex: "FFD6D6")
    }
    var rankBgColor: Color {
        if !submitted { return Color(hex: "E5E7EB") }
        return isCorrect ? Color.green : Color.red
    }
    var rankTextColor: Color {
        if !submitted { return Color(hex: "6B7280") }
        return .white
    }
...
// MARK: - STEP 4 PLACEHOLDER (ersetze durch echte View)
// MARK: - STEP 4: Bildaufgabe / Diagnose
struct Step4DiagnosisView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ProgressBarView(step: 4, total: 7, score: submitted ? 10 : 0)
                ...
                VStack(spacing: 10) {
                    ForEach(imageOptions, id: \.id) { opt in
                        ...
                        Button(action: { if !submitted { if isSel { selectedImages.remove(opt.id) } else { selectedImages.insert(opt.id) } } }) {
                            HStack(spacing: 12) {
                                ...
                                Text(opt.label)
                                    ...
                                    .frame(maxWidth: .infinity, alignment: .leading)
                                Image(systemName: submitted ? (isRight ? "checkmark.circle.fill" : (isWrong ? "xmark.circle.fill" : "circle")) : (isSel ? "checkmark.circle.fill" : "circle"))
                            ...
                            .overlay(RoundedRectangle(cornerRadius: 14)
                                .stroke(isRight ? Color.green : (isWrong ? Color.red : (isSel ? Color(hex: "084B83") : Color(hex: "E5E7EB"))), lineWidth: isSel ? 2 : 1.5))
                        }
                        .buttonStyle(PlainButtonStyle())
                    }
                }
                ...
                if submitted {
                    ExpertCommentView(score: 10, text: "Replantation so schnell wie möglich! Die Schiene stabilisiert den Zahn. Röntgen zur Verlaufskontrolle ist Pflicht.")
        ...
        .navigationTitle("Schritt 4 von 7")
        .navigationBarTitleDisplayMode(.inline)
        .background(Color(hex: "F9FAFB"))
        .animation(.easeInOut, value: submitted)
    }
...
// MARK: - QuestionCardView (used by Steps 5–8)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text(subtitle)
                    .font(.system(size: 13))
                    .foregroundColor(Color(hex: "424750"))
            }
...
// MARK: - ConfirmButton (used by Steps 5–8)
...
// MARK: - confirmButtonLabel (free func, used by Steps 5–8)
...
// MARK: - Step7NavBar (Bottom nav shown in Step 7)
...
struct Step5DiagnosisView: View {
    ...
    let correct: [String: String] = [
        "11": "Komplette Avulsion",
        "12": "Laterale Dislokation",
        "21": "Kronenfraktur mit Pulpabeteiligung",
        "22": "Keine Verletzung"
    ]
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ProgressBarView(step: 5, total: 7, score: submitted ? score : 0)

                QuestionCardView(
                    badge: "Diagnose",
                    badgeIcon: "stethoscope",
                    badgeColor: Color(hex: "FFD6A5"),
                    typeLabel: "ZUORDNUNGSAUFGABE",
                ...
                if submitted {
                    ExpertCommentView(
                        score: score,
                        text: "Waren Sie sich bei den Zähnen 12 und 22 nicht sicher? Gerade bei nicht kieferorthopädisch vorbehandelten Patienten ist es schwer, laterale Dislokationen zu identifizieren. Unser Patient fiel hauptsächlich auf die rechte Gesichtshälfte – daher ist es wahrscheinlich, dass Zahn 12 etwas abbekommen hat.",
                    )
                }
        ...
        .navigationTitle("Schritt 5 von 7")
        .navigationBarTitleDisplayMode(.inline)
...
// FIX: Extracted tooth row into its own View to avoid closure capture issues
private struct ToothRowView: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            ZStack {
                Circle()
                    .fill(Color(hex: "00345f").opacity(sel != nil ? 1.0 : 0.8))
                    .frame(width: 48, height: 48)
                    .overlay(
                        Circle().stroke(
                            submitted
                                ? (isRight ? Color.green : (isWrong ? Color.red : Color.clear))
                                : Color.clear,
                            lineWidth: 2.5
                        )
                    )
        ...
        .background(
            submitted
                ? (isRight ? Color(hex: "CAFFBF") : Color(hex: "FFD6D6"))
                : Color.white
        )
        .clipShape(RoundedRectangle(cornerRadius: 16))
        .overlay(
...
// Reused confirmButtonLabel is defined earlier to avoid duplicates

// MARK: - STEP 6: Repositionierung (Sortierung)
...
struct Step6SortingTherapyView: View {
    ...
    var allCorrect: Bool {
        items.enumerated().allSatisfy { $0.element.correctRank == $0.offset + 1 }
    }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ProgressBarView(step: 6, total: 7, score: submitted ? score : 0)

                QuestionCardView(
                    badge: "Therapie",
                    badgeIcon: "arrow.up.arrow.down.circle.fill",
                    badgeColor: Color(hex: "C9F0FF"),
                    typeLabel: "SORTIERAUFGABE",
                    question: "Wie gehen Sie bei der Repositionierung des avulsierten Zahnes vor? Bringen Sie die Schritte in die richtige Reihenfolge.",
                ...
                VStack(spacing: 12) {
                    ForEach(Array(items.enumerated()), id: \.element.id) { offset, item in
                        let rank    = offset + 1
                        let correct = submitted && rank == item.correctRank
                        ...
                        SortCardView(
                            ...
                            onTap: {
                                withAnimation(.spring(response: 0.25)) {
                                    activeID = (activeID == item.id) ? nil : item.id
                                }
                            },
                            onMoveUp:   { moveUp(item) },
                            onMoveDown: { moveDown(item) }
                        ...
                        .onDrop(
                            of: [.text],
                            delegate: Step6DropDelegate(item: item, items: $items, dragging: $dragging)
                        )
                    }
                }
                .padding(.horizontal, 16)
        ...
        .navigationTitle("Schritt 6 von 7")
        .navigationBarTitleDisplayMode(.inline)
        .background(Color(hex: "F9FAFB").ignoresSafeArea())
        .animation(.easeInOut, value: submitted)
    }
...
// FIX: Extracted sort card to avoid closure-capture issues inside ForEach
private struct SortCardView: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            ZStack {
                Circle()
                    .fill(submitted ? (isCorrect ? Color.green : Color.red) : Color(hex: "00345f"))
            ...
            if !submitted {
                VStack(spacing: 0) {
                    Button(action: onMoveUp) {
                        Image(systemName: "chevron.up")
                            .font(.system(size: 14, weight: .semibold))
                            .foregroundColor(Color(hex: "727781").opacity(0.6))
        ...
        .background(
            submitted
                ? (isCorrect ? Color(hex: "CAFFBF") : Color(hex: "FFD6D6"))
                : Color.white
        )
        .clipShape(RoundedRectangle(cornerRadius: 14))
        .overlay(
            RoundedRectangle(cornerRadius: 14).stroke(
                submitted
                    ? (isCorrect ? Color.green : Color.red)
                            : (isActive ? Color(hex: "00345f") : Color(hex: "727781")),
                lineWidth: (isActive || submitted) ? 2 : 1.5
            )
        )
        .shadow(
            color: Color(hex: "00345f").opacity(isActive ? 0.18 : 0.05),
...
// MARK: - STEP 7: Single Choice MCQ + BottomNavBar
struct Step7MCQView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ScrollView {
                VStack(spacing: 20) {
                    ProgressBarView(step: 7, total: 7, score: submitted ? 10 : 0)

                    QuestionCardView(
                        badge: "Behandlung",
                        badgeIcon: "cross.case.fill",
                        badgeColor: Color(hex: "CAFFBF"),
                    ...
                    if submitted {
                        ExpertCommentView(
                        score: score,
                        text: "Bei abgeschlossenem Wurzelwachstum ist die Vitalexstirpation (Entfernung der gesamten Pulpa) indiziert, gefolgt von einer Wurzelkanalbehandlung. Die partielle Pulpotomie ist eher bei jungen Zähnen mit offenem Apex angezeigt.",
                        )
                    }
            ...
            VStack(spacing: 0) {
                Divider()
                Step7NavBar(selected: $navTab)
                    .background(Color(hex: "f8f9ff"))
            }
        }
        .navigationTitle("Schritt 7 von 7")
        .navigationBarTitleDisplayMode(.inline)
        .background(Color(hex: "F9FAFB").ignoresSafeArea())
        .animation(.easeInOut, value: submitted)
    }
...
// FIX: Extracted MCQ row to avoid complex closure chains inside ForEach
            ...
            HStack(spacing: 12) {
                ...
                ZStack {
                    ...
                    if isSelected && !submitted {
                        Circle().fill(Color(hex: "00345f")).frame(width: 12, height: 12)
                    }
                    if submitted && isCorrect {
...
struct Step8PhotoUploadView: View {
    ...
    var body: some View {
                ...
                VStack(spacing: 20) {
                        ...
                        GeometryReader { geo in
                            ZStack(alignment: .leading) {
                                RoundedRectangle(cornerRadius: 4)
                                    .fill(Color(hex: "2e6385"))
                                    .frame(height: 6)
                                RoundedRectangle(cornerRadius: 4)
                                    .fill(Color(hex: "00345f"))
                    ...
                    HStack {
                        Label("Dokumentation", systemImage: "camera.fill")
                            .font(.system(size: 13, weight: .bold))
                            .foregroundColor(Color(hex: "0c4b6c"))
                            .padding(.horizontal, 14).padding(.vertical, 7)
                            .background(Color(hex: "FFD6A5"))
                            .clipShape(Capsule())
                            .opacity(isPulsing ? 1.0 : 0.6)
                            // FIX: animation bound to isPulsing Bool state
                            .animation(
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 16)
                            .stroke(Color(hex: "727781").opacity(0.4), lineWidth: 1)
                    )
                    .shadow(color: Color(hex: "00345f").opacity(0.08), radius: 12, y: 4)
                    .padding(.horizontal, 16)

                    // Photo grid — 5 slots, 2-column, 1:1 aspect
                    LazyVGrid(
                    ...
                    DrLukasHintView(text: "Klinische Fotografie: von vorne, seitlich und von oben fotografieren. So siehst du Schiene, Reposition und Wundversorgung!")
                        ...
                        VStack(spacing: 12) {
                            ...
                            Text("Fall abgeschlossen! 🎉")
                                .font(.system(size: 22, weight: .bold))
                                    .foregroundColor(Color(hex: "00345f"))
                            Text("Deine Bilder wurden eingereicht. Du erhältst nach Bewertung deine Punkte.")
                                .font(.system(size: 14))
                                .foregroundColor(Color(hex: "0b1c30"))
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 10) {
                    ConfirmButton(
                        ...
                        icon: "trophy.fill",
                        enabled: !images.isEmpty && !submitted
                    ) {
                        withAnimation(.easeInOut) { submitted = true }
                    }
                    if !submitted {
                        Text("Du erhältst nach Bewertung deine Punkte")
                            .font(.system(size: 12))
                            .foregroundColor(Color(hex: "0b1c30"))
                    }
                }
        ...
        .navigationTitle("Schritt 8 – Abschluss")
        .navigationBarTitleDisplayMode(.inline)
        .background(Color(hex: "F9FAFB").ignoresSafeArea())
        .sheet(isPresented: $showPicker) {
            ImagePicker(images: $images, maxCount: maxPhotos)
        }
        .animation(.easeInOut, value: submitted)
        // FIX: trigger animation on appear, not inside body
        .onAppear { isPulsing = true }
    }
}

// FIX: Extracted photo slot to avoid @State mutation inside ForEach body
        ...
        if let img = image {
            ZStack(alignment: .topTrailing) {
                Image(uiImage: img)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 14)
                            .stroke(Color(hex: "727781"), lineWidth: 1)
                    )
